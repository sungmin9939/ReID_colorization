...
def main(opt):
    ...
    #make model
    np_model = Trans_Colorization(opt).to(device)

    
    if os.path.exists(opt.checkpoint):
        np_model.load_state_dict(torch.load('./checkpoint/start_color.pth'))
    if opt.resume_epoch:
        print("load resuma checkpoint from {}".format(checkpoint_dir))
    ...
    if opt.optim == 'sgd':
        ...
        optimizer = optim.AdamW([{'params': np_model.thermal_encoder.parameters(), 'lr':0.0001},
                                 {'params': np_model.thermal_decoder.parameters()}], lr=opt.lr, weight_decay=opt.decay)
